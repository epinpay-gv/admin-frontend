...
import { usePaymentCountryStatus } from "@/features/payment/hooks/usePaymentCountryStatus";
...
interface PaymentCountryStatusModalProps {
  open: boolean;
  onClose: () => void;
...
export default function PaymentCountryStatusModal({
  open,
  onClose,
  method,
  onUpdate,
}: PaymentCountryStatusModalProps) {
  const {
    forbidden,
    isForbidden,
    toggleCountry,
    ...
    countries,
    countriesLoading,
  } = usePaymentCountryStatus(method);
        ...
        <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
          ...
          >
            <Command style={{ background: "var(--background-secondary)" }}>
              <CommandInput placeholder="Ülke ara..." style={{ color: "var(--text-primary)" }} />
              <CommandList>
                <CommandEmpty
                    ...
                    {countries.map((country) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <Check
                            size={13}
                            className={cn("transition-opacity", banned ? "opacity-100 text-red-400" : "opacity-0")}
                          />
                        </CommandItem>
                      );
                    })}
